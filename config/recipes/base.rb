...
namespace :deploy do
  ...
  task :install do
    ...
    run "#{sudo} wget http://www.imagemagick.org/download/legacy/ImageMagick-6.7.7-10.tar.gz"
    run "#{sudo} tar xvfz ImageMagick-6.7.7-10.tar.gz"
    run "cd ImageMagick-6.7.7-10 && #{sudo} ./configure && #{sudo} make"
    run "cd ImageMagick-6.7.7-10 && #{sudo} make install"
